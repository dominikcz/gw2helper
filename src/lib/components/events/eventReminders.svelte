<script>
	...
	import { __ } from '$lib/localizer.js';
...
<h2>{__(langCode, 'events.watched.watched')}</h2>
<EventsList events={getWatched(version)} on:toggle-watched={hndToggleWatched} on:alarms-change={hndAlarmsChange}>
	{@html __(langCode, 'events.watched.empty_list', { img_url: `${base}/assets/rewards/map_heart_empty.png` } )}
</EventsList>

<fieldset class="settings">
	<legend>{__(langCode, 'events.watched.reminder_settings')}</legend>
	<input type="range" name="vol" min="0" max="10" step="1" bind:value={inAdvance} />
	<p>{__(langCode, 'events.watched.notify_me', {inAdvance})}</p>
	<div class="group">
		<h4>{__(langCode, 'events.watched.alarm_sound')}</h4>
	...
	<button on:click={() => testAlarm()}>{__(langCode, 'events.watched.test_alarm', { sound } )}</button>
	<button on:click={saveNotifySettings}>save</button>
</fieldset>

<h2>{__(langCode, 'events.available')}</h2>
<section>
	<SearchInput bind:value={filter} name="filter" id="filter" placeholder={__(langCode, 'common.list_too_long')} />
</section>
