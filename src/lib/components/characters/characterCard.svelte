<script lang="ts">
	...
	interface Props {
		char: ApiCharacterDto;
	}

	let { char }: Props = $props();

	const days = $derived(helperUtils.tillBirthday(char.created || new Date().toISOString()));
	const gender = $derived((char.gender || '').toLowerCase());
...
<details use:grungeBorder open>
	...
	<article class="character">
		...
		<section>
			<div class="sect-prof">
				<img src={asset('/assets/Present_quaggan_icon.png')} alt="present" style:width={iconScale(char.created || new Date().toISOString())} />
			</div>
			<div class="sect-info">
		...
		<section>
			<div class="sect-prof">
				<img src={asset('/assets/Grave_Finisher.png')} alt="gravestone" />
			</div>
			<div class="sect-info">
				...
				<div class="info">({deathsPerHour()}/h)</div>
			</div>
		</section>
	</article>
</details>
